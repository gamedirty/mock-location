...
**悬浮微调按钮**
- 一个能浮在**别的应用上面**的小方块：拖动把手挪位置，3×3 方向键推坐标，
  中间那颗显示当前步长、点一下切换（1m → 10m → 100m → 1km）
- 用法：主界面点开「悬浮微调按钮」→ 授权「显示在其他应用上层」→ 开始模拟后它就会出现，
  然后你切到任何应用里都能直接调，不用来回切回本应用
- 拖动后的位置会记住，下次还在原地；按 × 关掉它就等于关掉这个开关
...
先在应用里点「取一次位置看看」自检——它会按普通应用的方式向系统要一次位置，告诉你系统
发回来的到底是不是模拟坐标、有没有被标记成「模拟」。结果分两种：

- **自检显示是模拟坐标、带「模拟」标记，但目标应用仍显示真实位置** → 这个应用自己识别并
  拒绝了模拟位置，属于应用侧的反模拟。典型例子是**高德地图**：它的 Android 定位 SDK 从
  **6.5.1** 起新增了虚拟定位检测，检测到模拟位置直接返回**定位失败（错误码 15）**，要用
  `setMockEnable(true)` 才能关闭，而高德地图 App 没关。它识别出模拟位置后就不用了，
  转头用自己那套网络定位（WiFi 热点指纹 / 基站）算回真实位置，于是你看到蓝点还在原地。

  这类识别发生在应用那一侧，普通应用拿不到、也改不掉系统打在位置上的模拟标记，
  **没有任何非 root 的办法能绕过**，本应用也不做这些。

- **系统发回来的本来就是真实位置** → 模拟没生效，回去检查：本应用是否还被选为
  「模拟位置信息应用」（系统更新或 ROM 清理会把这个设置重置）、系统定位总开关是否打开、
  然后停止再重新开始一次模拟。

能提高成功率的一般是这三件事：① 关掉 WiFi（以及「WLAN 扫描」「蓝牙扫描」，高德的网络定位
主要靠 WiFi 热点指纹）；② 把系统定位模式改成「仅设备/GPS」；③ 把目标应用强行停止、
清掉缓存再打开。应用里那个「打开系统定位设置」按钮可以直接跳到这一步。

实测下来，用系统 `LocationManager` 取位置、且没做反模拟校验的应用（大部分打卡类、
运动记录类、自己写的测试 App）都能正常拿到模拟位置。
...
对标 Google Play 上那类假定位工具的招牌功能：在目标应用保持前台的情况下直接推位置。
拖动把手挪位置，3×3 方向键微调，中间按钮切步长，坐标实时显示在悬浮块上。
需要「显示在其他应用上层」权限（点开关时应用会引导授权）。已在 Android 14 上验证：
浮在桌面上，点 3 次「→」精确移动 30 米，切到 100 米档后一次移动 100 米。

同时修了个小问题：用悬浮按钮或收藏切换坐标后，主界面的经纬度输入框现在会自动跟着同步。

**1.0.2** —— 新增「自检」，用来判断问题出在哪一侧

- 新增自检卡片：按普通应用的方式向系统要一次位置，逐 provider 显示拿到的坐标、
  精度，以及**有没有被标记成「模拟」**，并给出结论
- 新增「复制诊断信息」：设备型号 / 系统版本 / 授权状态 / provider 列表 / 自检结果 / 日志，
  一键复制，方便排查
- 新增「打开系统定位设置」快捷入口（关掉 WiFi 辅助定位用）
- 应用内说明了应用侧反模拟（高德 SDK 6.5.1+ 的错误码 15）这件事，不再让人猜

**1.0.1** —— 修掉「开发者选项里找不到本应用」

系统那个「选择模拟位置信息应用」的列表，是**按应用有没有在清单里声明
`android.permission.ACCESS_MOCK_LOCATION` 来筛选的**（系统的 `AppPicker` 就是拿这个权限去查）。
1.0 漏了这个声明，所以用 adb 授权限能跑，但你从界面里永远找不到它。
1.0.1 补上了。这个权限本身早就废弃、也不会真的被授予，纯粹是给系统列表用的"报名表"。
